...
// Enhanced Landmark Smoother for better mobile tracking
class LandmarkSmoother {
  constructor(posAlpha = 0.45, rotAlpha = 0.35) {
    this.posAlpha = posAlpha;
    this.rotAlpha = rotAlpha;
  ...
  smooth(current, deadzone = 0, deltaTime = 16) {
    if (!this.prev) {
      this.prev = { ...current };
      return { ...current };
    }
    ...
    const adaptivePosAlpha = Math.min(0.75, Math.max(0.35, this.posAlpha * (1 + speed / 15)));
...
function extractFaceGeometry(lm, W, H, useIris = true) {
  ...
  // Get face width from cheek landmarks for better scaling
  const leftFaceEdge = px(LANDMARKS.LEFT_FACE_EDGE);
  const rightFaceEdge = px(LANDMARKS.RIGHT_FACE_EDGE);
  const faceWidth = dist(leftFaceEdge, rightFaceEdge);
  const faceWidthMultiplier = Math.min(1.35, Math.max(0.85, faceWidth / (eyeSpan * 3.5)));
  ...
  const spanMult = useIris ? 1.0 : 1.35;
  let glassesWidth = eyeSpan * 2.0 * spanMult * faceWidthMultiplier;
  let glassesHeight = eyeSpan * 0.75 * spanMult * faceWidthMultiplier;
  
  // Clamp to reasonable values
  glassesWidth = Math.min(W * 0.8, Math.max(80, glassesWidth));
  glassesHeight = Math.min(H * 0.4, Math.max(40, glassesHeight));

  const avgZ = (leftIris.z + rightIris.z + (noseBridgeTop.z ?? 0)) / 3;
  const depthScale = Math.max(0.92, Math.min(1.08, 1 + (-avgZ * 0.6)));

  return { centerX, centerY, angle, glassesWidth, glassesHeight, depthScale, faceWidthMultiplier };
}
...
const TryOn = () => {
  ...
  // Draw loop with improved performance
  const drawLoop = useCallback(() => {
    rafIdRef.current = requestAnimationFrame(drawLoop);
    ...
    const lm = result.multiFaceLandmarks[0];
    const useRefined = !mobile || (lm.length > 473);
    const geo = extractFaceGeometry(lm, W, H, useRefined);
    ...
    let w = mobile ? smoothed.gw * adj.scaleW : smoothed.gw * adj.scaleW * smoothed.ds;
    let h = mobile ? smoothed.gh * adj.scaleH : smoothed.gh * adj.scaleH * smoothed.ds;
    
    // Apply face width multiplier for better fit on mobile
    if (mobile) {
      w *= (geo.faceWidthMultiplier || 1.0);
      h *= (geo.faceWidthMultiplier || 1.0);
    }
    w *= sSc;
    h *= sSc;
